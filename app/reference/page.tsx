...
export const metadata: Metadata = {
  title: { absolute: "Reference | Elektro projekti Maribor in okolica | El-Janko Mont" },
  description:
    ...
    <>
      ...
      <section style={s("background:#fff")}>
        <div style={s("max-width:1200px;margin:0 auto;padding:88px 24px")}>
          <div style={s("display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:22px")}>
            {REFERENCE.map((r) => (
              <div key={r.naslov} style={s("background:#F5F6F8;border:1px solid rgba(2,8,23,.06);border-radius:20px;overflow:hidden;display:flex;flex-direction:column")}>
                ...
                <div style={s("padding:24px 24px 26px;display:flex;flex-direction:column;gap:10px;flex:1")}>
                  <div style={s("display:flex;flex-wrap:wrap;gap:8px")}>
                    <span style={s("font:600 11.5px 'Space Grotesk',sans-serif;letter-spacing:.08em;text-transform:uppercase;background:#020817;color:#FFB400;border-radius:999px;padding:6px 12px")}>{r.tip}</span>
                    <span style={s("font:600 11.5px 'Space Grotesk',sans-serif;letter-spacing:.08em;text-transform:uppercase;background:#fff;border:1px solid rgba(2,8,23,.12);color:#47536B;border-radius:999px;padding:6px 12px")}>{r.lokacija}</span>
                  </div>
      ...
      <section style={s("background:#F5F6F8")}>
        <div style={s("max-width:820px;margin:0 auto;padding:88px 24px;text-align:center")}>
          <span style={s("width:52px;height:52px;border-radius:14px;background:#020817;display:inline-flex;align-items:center;justify-content:center;margin-bottom:20px")}>
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#FFB400" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l8 3v6c0 4.8-3.4 7.7-8 9-4.6-1.3-8-4.2-8-9V6z"></path><path d="M9 12l2 2 4-4.5"></path></svg>
          </span>
          <h2 style={s("font:700 clamp(26px,3.2vw,36px)/1.15 'Space Grotesk',sans-serif;letter-spacing:-0.02em;margin:0 0 16px")}>
            Vsak projekt izvedemo z mislijo na <span style={s("background:linear-gradient(transparent 58%,#FFB400 58%)")}>varnost</span>
          </h2>
      ...
      <section style={s("background:#fff")}>
        <div style={s("max-width:1200px;margin:0 auto;padding:88px 24px")}>
          <div style={s("background:#020817;background-image:linear-gradient(rgba(255,255,255,.045) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.045) 1px,transparent 1px);background-size:46px 46px;border-radius:24px;padding:64px 32px;text-align:center;position:relative;overflow:hidden")}>
